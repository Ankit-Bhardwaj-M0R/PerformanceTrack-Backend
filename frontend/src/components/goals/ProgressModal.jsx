...
export default function ProgressModal({ isOpen, onClose, goal, form, setForm, onSubmit, submitting }) {
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Add Progress Update">
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <div>
          <label className="form-label">Progress Percentage: {form.progressPercentage}%</label>
          <input
            type="range" min="0" max="100"
            value={form.progressPercentage}
            onChange={e => setForm({ ...form, progressPercentage: parseInt(e.target.value) })}
            className="w-full"
          />
