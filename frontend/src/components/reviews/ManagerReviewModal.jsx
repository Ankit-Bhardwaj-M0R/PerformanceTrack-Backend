 ...
 * Props:
 *  isOpen, onClose
 *  review              — selected review (used to show employee self-assessment)
 *  employeeName        — resolved display name (optional)
 *  form, setForm       — { managerFeedback, managerRating, ratingJustification, compensationRecommendations, nextPeriodGoals }
 *  onSubmit, submitting
 */
export default function ManagerReviewModal({ isOpen, onClose, review, employeeName, form, setForm, onSubmit, submitting }) {
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Submit Manager Review" size="lg">
      <form onSubmit={onSubmit} className="space-y-4">
        {employeeName && (
          <div className="bg-gray-50 rounded-lg p-3 text-sm">
            <p className="font-medium text-gray-700">
              Employee: <span className="text-blue-700 font-semibold">{employeeName}</span>
            </p>
          </div>
        )}
        ...
        {review?.selfAssessment && (
          <div className="bg-blue-50 rounded-lg p-3 text-sm">
            ...
            {review.employeeSelfRating && (
              <p className="text-xs text-blue-500 mt-1">Self Rating: {review.employeeSelfRating}/5</p>
            )}
          </div>
        )}
        ...
        <div>
          ...
          <div className="mt-2">
            <StarRating value={form.managerRating} onChange={v => setForm({ ...form, managerRating: v })} />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              onChange={e => setForm({ ...form, compensationRecommendations: e.target.value })}
