...
import type { PasswordFormState, UserRole } from '../types'
...
export default function ProfilePage(): JSX.Element {
  ...
  const handlePasswordChange = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      toast.error(axiosError.response?.data?.msg || 'Failed to change password. Check your current password.')
    } finally {
      setSubmitting(false)
    }
  }
            ...
            <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  {user?.role ? roleDescriptions[user.role as UserRole] ?? '' : ''}
                </p>
              </div>
            </div>
