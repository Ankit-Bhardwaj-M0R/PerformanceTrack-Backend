...
import type { DashboardMetrics, ReviewCycle } from '../../types'
  ...
  const loadData = async (): Promise<void> => {
    ...
    try {
      ...
      if (cyclesRes.status === 'fulfilled') {
        const raw = cyclesRes.value as ReviewCycle[] | { content?: ReviewCycle[] }
        const list: ReviewCycle[] = Array.isArray(raw) ? raw : (raw as { content?: ReviewCycle[] }).content ?? []
        setActiveCycles(list.filter((c: ReviewCycle) => c.status === 'ACTIVE'))
      }
    } catch {
