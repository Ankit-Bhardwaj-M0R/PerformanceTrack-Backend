...
import type { UserDTO, GoalResponseDTO, PerformanceReviewResponseDTO } from '../../types'
...
export default function TeamMembersPage(): JSX.Element {
  ...
  const [memberGoals, setMemberGoals]       = useState<GoalResponseDTO[]>([])
  const [memberReviews, setMemberReviews]   = useState<PerformanceReviewResponseDTO[]>([])
  ...
  const loadTeam = async (): Promise<void> => {
    ...
    try {
      ...
      const list: UserDTO[] = (raw as { content?: UserDTO[] })?.content || (raw as UserDTO[]) || []
      setMembers(Array.isArray(list) ? list : [])
    } catch {
      // fallback: try getAllUsers and filter
      try {
        const data = await userService.getAllUsers(0, 100)
        const raw2 = data as unknown
        const list2: UserDTO[] = (raw2 as { content?: UserDTO[] })?.content || (raw2 as UserDTO[]) || []
        setMembers(list2.filter((u: UserDTO) => u.role === 'EMPLOYEE'))
      } catch {
  ...
  const departments: string[] = [...new Set(members.map(m => m.department).filter((d): d is string => Boolean(d)))]
  ...
  const openDetail = async (member: UserDTO): Promise<void> => {
    ...
    try {
      ...
      if (goalsRes.status === 'fulfilled') {
        const all: GoalResponseDTO[] = ((goalsRes.value as unknown as { content?: GoalResponseDTO[] })?.content) || (goalsRes.value as unknown as GoalResponseDTO[]) || []
        setMemberGoals(all.filter(g => g.assignedToUserId === member.userId))
      }
      if (reviewsRes.status === 'fulfilled') {
        const all: PerformanceReviewResponseDTO[] = ((reviewsRes.value as unknown as { content?: PerformanceReviewResponseDTO[] })?.content) || (reviewsRes.value as unknown as PerformanceReviewResponseDTO[]) || []
            ...
            <div className="flex items-center gap-4 p-4 bg-purple-50 rounded-xl">
              ...
              <div>
                ...
                <div className="flex gap-2 mt-2 flex-wrap">
                  <span className="text-xs bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full font-medium">{selectedMember.role}</span>
                  {selectedMember.department && (
                    <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">{selectedMember.department}</span>
                  )}
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${selectedMember.status === 'ACTIVE' || !selectedMember.status ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
