...
import type { DashboardMetrics, GoalResponseDTO, PerformanceReviewResponseDTO } from '../../types'
...
function MetricCard({ title, value, icon: Icon, color, subtitle, onClick }: MetricCardProps): JSX.Element {
  return (
    <div
      className={`metric-card ${onClick ? 'cursor-pointer hover:shadow-md' : ''} transition-all`}
      onClick={onClick}
    >
...
export default function ManagerDashboard(): JSX.Element {
  ...
  const [recentGoals, setRecentGoals]   = useState<GoalResponseDTO[]>([])
  const [pendingReviews, setPendingReviews] = useState<PerformanceReviewResponseDTO[]>([])
  ...
  const loadData = async (): Promise<void> => {
    ...
    try {
      ...
      if (goalsRes.status === 'fulfilled') {
        const raw = goalsRes.value as { content?: GoalResponseDTO[] } | GoalResponseDTO[]
        const list: GoalResponseDTO[] = Array.isArray(raw) ? raw : (raw as { content?: GoalResponseDTO[] }).content ?? []
        setRecentGoals(list)
      }
      if (reviewsRes.status === 'fulfilled') {
        const raw = reviewsRes.value as { content?: PerformanceReviewResponseDTO[] } | PerformanceReviewResponseDTO[]
  ...
  return (
    <Layout title="Manager Dashboard">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <MetricCard title="Total Members"    value={totalMembers}      icon={Users}       color="text-purple-600" subtitle="Active team members"      onClick={() => navigate('/team-members')} />
        <MetricCard title="Open Goals"       value={openGoals}         icon={Target}      color="text-blue-600"   subtitle="Active team goals"         onClick={() => navigate('/team-goals')} />
        <MetricCard title="Pending Approvals" value={pendingApprovals} icon={Clock}       color="text-orange-600" subtitle="Goals awaiting review"     onClick={() => navigate('/team-goals')} />
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <MetricCard title="Completed Goals"     value={completedGoals}     icon={CheckCircle}  color="text-green-600"  subtitle="Successfully finished"       onClick={() => navigate('/team-goals')} />
        <MetricCard title="Pending Completions" value={pendingCompletions} icon={TrendingUp}   color="text-indigo-600" subtitle="Awaiting completion review"   onClick={() => navigate('/team-goals')} />
        <MetricCard title="Pending Reviews"     value={pendingReviews.length} icon={ClipboardList} color="text-red-600" subtitle="Performance reviews pending" onClick={() => navigate('/reviews')} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">
                      {goal.category} · {goal.assignedToUserName || 'Team Member'}
                    </p>
                  </div>
        ...
        <div className="card">
          ...
          <div className="space-y-3">
            {pendingApprovals > 0 && (
              <div className="flex items-center gap-3 p-3 bg-yellow-50 border border-yellow-100 rounded-lg cursor-pointer hover:bg-yellow-100 transition-colors" onClick={() => navigate('/team-goals')}>
            ...
            {pendingCompletions > 0 && (
              <div className="flex items-center gap-3 p-3 bg-orange-50 border border-orange-100 rounded-lg cursor-pointer hover:bg-orange-100 transition-colors" onClick={() => navigate('/team-goals')}>
                <div className="w-8 h-8 bg-orange-100 rounded-lg flex items-center justify-center"><CheckCircle size={16} className="text-orange-600" /></div>
                <div>
