...
export default function EmployeeDashboard(): JSX.Element {
  ...
  const completionRate  = metrics?.completionRate   ?? (totalGoals > 0 ? Math.round(((completedGoals as number) / (totalGoals as number)) * 100) : 0)
        ...
        <div className="card">
          ...
          <div className="space-y-3">
            ...
            {pendingReviews.filter(r => r.status === 'COMPLETED').length > 0 && (
              <div
                className="flex items-center gap-3 p-3 bg-green-50 border border-green-100 rounded-lg cursor-pointer hover:bg-green-100 transition-colors"
                onClick={() => navigate('/reviews')}
              >
            ...
            {(pendingGoals as number) === 0 && pendingReviews.length === 0 && (
              <div className="text-center py-8 text-gray-400">
